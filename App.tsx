...
export default function App() {
  ...
  const isPremiumRef = useRef<boolean>(true);
  ...
  useEffect(() => {
    ...
    const connectWebSocket = () => {
      ...
      if (
        ...
        // Fallback to the persistent Cloud Run Shared App URL for syncing, as static hosts do not run WebSocket servers
        wsUrl = "wss://ais-pre-7zpzhek72wbuzy24tstjds-612901282613.asia-southeast1.run.app/api/ws";
      } else {
      ...
      {!profile ? (
        <WelcomeScreen
          ...
          onJoin={(username, avatar, color, roomId, lat, lng, locName, premium, bio, country, language, favGenre, relationshipStatus) => {
            ...
            // Initialize immediate fallback roomState to avoid loading screen delays
            const currentUid = getOrGenerateUserId();
            setRoomState({
              roomId: roomId.toUpperCase(),
              videoUrl: "https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/Sintel.mp4",
              videoTitle: "Sintel (Fantasy Animation Movie)",
              videoType: "direct",
              isPlaying: false,
              currentTime: 0,
              ...
              chatHistory: [
                {
                  ...
                  avatar: "🍿",
                  color: "#9CA3AF",
                  text: `Welcome, ${username}! Share your Room Code (${roomId.toUpperCase()}) with your partner to start sync-watching.`,
                  timestamp: Date.now()
                }
              ],
              hostId: currentUid,
        ...
        <PartyRoom
          ...
          onLocalRoomStateUpdate={(updatedFields) => {
            setRoomState((prev) => prev ? { ...prev, ...updatedFields } : null);
          }}
          isInVoice={isInVoice}
          setIsInVoice={(val) => {
