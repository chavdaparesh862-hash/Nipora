...
async function startServer() {
  ...
  const upload = multer({
    storage,
    limits: { fileSize: 4000 * 1024 * 1024 } // Support up to 4GB uploads (as configured in client)
  });
  ...
  app.post("/api/upload", (req, res, next) => {
    ...
    try {
      ...
      const isPremium = room ? !!room.isPremiumEnabled : false;
      const maxLimit = isPremium ? 4000 * 1024 * 1024 : 1024 * 1024 * 1024; // 4GB for premium, 1GB for free
      ...
      if (req.file.size > maxLimit) {
        ...
        return res.status(400).json({
          error: isPremium
            ? "Video exceeds the maximum Premium limit (4GB)."
            : "Video exceeds the free room limit (1GB). Please upgrade this room to Premium to upload files up to 4GB!"
        });
      }
          ...
          case "video_control": {
            ...
            if (action === "play") {
              ...
              if (room.hostId !== currentUserId) {
                console.log(`[Server] Unauthorized video change attempt by non-host ${currentUserId}`);
                return;
              }
              room.videoUrl = url;
